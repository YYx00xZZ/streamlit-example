...
def main():
	...
	def equalize_events(dataframe):
		...
		df_alex_under = pd.concat([df_class_33024_undersample, df_class_33025, df_class_33026],
								  axis=0)  # това е тест сет
	...
	def build_forest_model(df):
		...
		numbers = st.text_input(
			"PLease enter 14 values (separated with ,). THis represent 1 row of BCI data for which we`ll predict if it's left or right.")
		st.text(
			'for e.g. #4081.0256, 4091.6667, 4069.6155, 4088.4614, 4099.1025, 4092.3076, 4078.7180, 4078.2051, 4066.7949, 4066.4102, 4046.7949, 4050, 4092.9487, 4081.2820')
		if not numbers:
	...
	if classifier == "Random Forest":
		...
		with st.sidebar.subheader('2.1. Learning Parameters'):
			...
			parameter_max_features = st.sidebar.select_slider('Max features (max_features)',
	...
	if st.button('predict'):
		...
		for row in df_to_predict.values:
			...
			prediction = loaded_knn_model.predict(
				np.array(row).reshape(1, -1))  # np.array(collect_numbers(row)).reshape(1,-1)
			# st.text(prediction)
